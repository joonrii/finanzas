...
export function NewTransactionForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // The database function performs the transaction and all balance updates
    // atomically. This prevents partial writes and race conditions.
    ...
    if (transactionError || !transactionId) {
      setError(transactionError?.message ?? "No se pudo guardar el movimiento.");
      setLoading(false);
      return;
    }
    ...
    if (selectedTagIds.length > 0) {
      ...
      if (tagsError) {
        // The financial transaction is already safe; only the optional tags failed.
        toast("Movimiento guardado, pero no se pudieron guardar las etiquetas");
      }
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div>
        <input type="number" inputMode="decimal" step="0.01" required autoFocus placeholder="0,00" value={amount}
      ...
      <div className="grid grid-cols-4 gap-2">
        {TYPES.map((t) => (
          <button type="button" key={t.value} onClick={() => setType(t.value)}
            className={clsx("rounded-xl py-2.5 text-xs font-medium border", type === t.value ? "bg-positive/10 border-positive text-positive" : "bg-surface border-border text-muted")}>{t.label}</button>
        ))}
      </div>

      <div>
        <label className="text-sm text-muted mb-1 block">Descripción / comercio</label>
        <input type="text" value={description} onChange={(e) => { setDescription(e.target.value); setSuggestionNote(null); }}
          onBlur={handleDescriptionBlur} placeholder="Ej. Mercadona, cena con amigos…"
        ...
        {suggestionNote && <p className="text-positive text-xs mt-1">✨ {suggestionNote}</p>}
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-2">
            {visibleCategories.map((c) => {
              ...
              return <button type="button" key={c.id} onClick={() => { setCategoryId(c.id); setSuggestionNote(null); }}
                className={clsx("flex flex-col items-center gap-1 rounded-xl py-3 border text-[11px]", categoryId === c.id ? "bg-positive/10 border-positive text-positive" : "bg-surface border-border text-muted")}>
                <span className={"w-7 h-7 rounded-full flex items-center justify-center text-sm " + color.bg}>{c.icon}</span>
                {c.name}
              </button>;
            })}
          </div>
      ...
      <div>
        <label className="text-sm text-muted mb-1 block">{needsDestination ? "Cuenta de origen" : "Cuenta"}</label>
      ...
      {needsDestination && <div>
        <label className="text-sm text-muted mb-1 block">Cuenta de destino</label>
